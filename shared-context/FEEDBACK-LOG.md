...
### 12. 文章必须有干货，不能是纯水文（2026-03-10 旭严厉反馈）
- 触发：OpenClaw一键部署文章v3，旭打5分（不及格）
- 问题：没亮点、没吸引力、纯水文、要干货没干货、要方法没方法、要价值没价值、说了一堆介绍
- 规则：每篇文章必须有至少1个可直接复用的方法/技巧/代码/模板，读者看完能立刻用上
- 「适合谁」不要写太窄（如"适合自媒体人"），OpenClaw人人都能用

### 13. 每篇文章必须有引流钩子（2026-03-10 已定稿）
- 规则：每篇公众号文章正文结束后必须加结尾钩子
- 模板文件：/root/.openclaw-002/workspace/templates/wechat-hook.md
- 直接复制模板内容粘贴到文章末尾，不得修改删减

### 14. 禁止emoji和字数统计（2026-03-10）
- 文章全文不要使用emoji表情符号
- 不要在文末加字数统计

### 15. 001是指挥不是执行（2026-03-10 旭明确）
- 001的职责：选题、写brief、审稿、打分、给修改意见、调度002
- 002的职责：写文章、配图、排版、发布到草稿箱
- 001绝不动手写文章、配图、发布，只负责指挥和质控
