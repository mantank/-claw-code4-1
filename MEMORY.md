...
- **2026-02-12** — 第六篇公众号写完+发布（首次有转化：3-5新关注+6-7分享）、公众号基建（合集+欢迎语）、AI日报优化方案+SOP完成、3篇文章入飞书知识库、Evolink域名更新

## 经验与教训

- 文章要用真实经历，不编造（第3篇从虚构改为真实事件，效果好很多）
- 工具够用就停，开始产出（"Ship over configure"）
- 图片生成优先级：Z-Image-Turbo（Evolink）> Gemini 2.0 Flash（Gemini 3 Pro额度用完了）
...
- Evolink: `sk-rkjDLqIIsfbvyHzFi9qpuaVODdO8xm3BtUAJF373D5yX4ulO`（图片生成，域名：api.evolink.ai，模型：nano-banana-2-lite / z-image-turbo，异步模式需查询task）
